...
According to our [contributing guidelines]({contributing_url}), **you don't need to wait to be assigned** to start working on an issue. We encourage you to:
...
We look forward to your contribution! If you have any questions, feel free to ask in the PR or check our [documentation]({contributing_url}).
    ...
    def auto_assign_issues_from_pr(self, pr_number, pr_author, pr_body, max_issues=10):
        ...
        try:
            ...
            if len(linked_issues) > max_issues:
                print(f'Found {len(linked_issues)} issue references, processing first {max_issues} to avoid rate limits')
                linked_issues = linked_issues[:max_issues]
                ...
                try:
                    ...
                    if current_assignees:
                        if pr_author in current_assignees:
                            print(f'Issue #{issue_number} already assigned to {pr_author}')
                        else:
                            print(f'Issue #{issue_number} already assigned to: {", ".join(current_assignees)}')
                        continue
                        
                    issue.add_to_assignees(pr_author)
                    assigned_issues.append(issue_number)
                    print(f'Assigned issue #{issue_number} to {pr_author}')
                    comment_message = f'This issue has been automatically assigned to @{pr_author} who opened PR #{pr_number} to address it. 🎯'
    ...
    def handle_pull_request(self):
        ...
        try:
            ...
            if action in ['opened', 'reopened']:
                assigned_issues = self.auto_assign_issues_from_pr(pr_number, pr_author, pr_body)
...
if __name__ == "__main__":
    ...
    # Try to load event payload from file if provided
    if len(sys.argv) > 1:
        try:
