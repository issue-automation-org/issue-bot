            ...
            for issue_number in linked_issues:
                try:
                    ...
                    if current_assignees and pr_author not in current_assignees:
                        print(f'Issue #{issue_number} is assigned to others: {", ".join(current_assignees)}')
                        continue
                    ...
                    if pr_author not in current_assignees:
                        ...
                        welcome_message = f'Welcome back, @{pr_author}! 🎉 This issue has been reassigned to you as you\'ve reopened PR #{pr_number}.'
...
class PRActivityBot:
    ...
    def handle_contributor_activity(self):
        ...
        try:
            pr_number = self.event_payload.get('issue', {}).get('number')
            commenter = self.event_payload.get('comment', {}).get('user', {}).get('login', '')
            ...
            for issue_number in linked_issues:
                try:
                    ...
                    # Only reassign if not already assigned to someone else
            ...
            encouragement_message = f'Thanks for following up, @{commenter}! 🙌 The stale status has been removed and the linked issue(s) have been reassigned to you. Looking forward to your updates!'
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if 'activity' in sys.argv[0]:
            bot = PRActivityBot()
        else:
            bot = PRReopenBot()
            
        # Load event payload from file if provided
        try:
